...
class Trainer:
    def     __init__(self, options):
        self.setup_seed(20)#种子
        self.opt = options
        self.log_path = os.path.join(self.opt.log_dir, self.opt.model_name) #E:\code\ADDS-DepthNet-main + model_name
        self.result_path = os.path.join(self.log_path, 'result.txt')  # E:\code\ADDS-DepthNet-main + model_name+result.txt
        ...
        self.num_input_frames = len(self.opt.frame_ids) #值为3
        ...
        if self.opt.only_depth_encoder:#如果设置，只训练深度编码器来检查私有和公共提取器是否可以得到纹理和结构
            self.opt.frame_ids = [0]

        self.models["encoder"] = networks.ResnetEncoder(
            self.opt.num_layers, self.opt.weights_init == "pretrained")#resent encoder
